...
from .serializers import PoseVideoSerializer
...
def extractKeypointImage(image):
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image.flags.writeable = False
        results = pose.process(image)
...
class SaveUploadedVideo(APIView):
    # permission_classes = [IsAuthenticated]
    def post(self, request, format=None):
        video = request.FILES.get("uploaded_video")
        unique_id =  uuid.uuid4()
        video2 = cv2.VideoCapture(video.temporary_file_path())
        file_path = "{}/videos/{}.mp4".format(settings.MEDIA_ROOT,unique_id) 
        ...
        for _ in tqdm(range(total_frames)):
            try:
                x = extractKeypointImage(video2.read()[1])
            except:
                continue
        ...
        with open(os.path.join(settings.MEDIA_ROOT, 'videos/{}.mp4'.format(unique_id)), "r+b") as f:
            model = PoseVideo.objects.create(user=request.user,uploaded_video=File(f, name="{}.mp4".format(unique_id)), original_video=video)
            serializer = PoseVideoSerializer(model, context={"request": request})
...
class Compare(APIView):
    def post(self, request, format=None):
        compare_video = request.FILES.get("compare") # جلب الفيديو المصور
        uploaded_id = request.POST.get("uploaded_id") # جلب الرمز الفريد

        uploaded_vedio = get_object_or_404(PoseVideo, id=uploaded_id) # جلب الريكورد من قاعدة البيانات لجلب الفيديو المرفوع

        video = cv2.VideoCapture(uploaded_vedio.uploaded_video.path) # ارساله لمكتبة opencv لعرضه
        ...
        for _ in tqdm(range(total_frames)):
            try:
                x = extractKeypointImage(video.read()[1])
            except:
                continue
                
            video_frames.append(x)
                
        # Number of frames after doing pose detection (frames with no pose detected have been skipped)
        video_length = len(video_frames)
        video.release()

  
        file_path = "{}/videos/{}.mp4".format(settings.MEDIA_ROOT,uploaded_id) # انشاء مسار الفيديو بعد المقارنة ووضع النتائج عليه
        camera = cv2.VideoCapture(compare_video.temporary_file_path()) # جلب مسار الفيديو المرفوع
        frame_width = int(camera.get(3))
        frame_height = int(camera.get(4))
        # fps = int(round(camera.get(5))) 
        size = (frame_width, frame_height)
        result = cv2.VideoWriter(file_path, 
                                # cv2.CAP_FFMPEG,
                                cv2.VideoWriter_fourcc(*'avc1'),
        ...
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence = 0.5) as pose:
            ...
            while camera.isOpened():
                ...
                # If no more frames are received from the camera
                if not camera_ret:
                    break
                    
                # Reset frame count to 0 when video has finished
                if i >= video_length:
                    i = 0
                ...
                image = cv2.cvtColor(camera_frame, cv2.COLOR_BGR2RGB)
                image.flags.writeable = False
                results = pose.process(image)
            ...
            with open(os.path.join(settings.MEDIA_ROOT, 'videos/{}.mp4'.format(uploaded_id)), "r+b") as f: # حفظه في قاعدة البيانات
                uploaded_vedio.compared_video = File(f, name="{}.mp4".format(uploaded_id)) # database
                uploaded_vedio.save() # database save
 
            serializer = PoseVideoSerializer(uploaded_vedio, context={"request": request}) # ارساله للمستخدم
        return Response(serializer.data)
